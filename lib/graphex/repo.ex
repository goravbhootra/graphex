defmodule Graphex.Repo do
  ...
  Ecto-like repository, which allows to embed the schema
    ...
    defmodule Repo do
      use Graphex.Repo, otp_app: :my_app, modules: [User]
    end
    ...
    config :my_app, Repo,
      ...
      port: 3306
  """
  alias Graphex.{Error, Node, Repo.Meta, Utils}
  ...
  defmacro __using__(opts) do
    quote bind_quoted: [opts: opts], location: :keep do
      ...
      def start_link(opts \\ []) do
        ...
        Graphex.Repo.Sup.start_link(start_opts)
      end

      def set(node, opts \\ []), do: Graphex.Repo.set(@name, node, opts)
      def set!(node, opts \\ []), do: Graphex.Repo.set!(@name, node, opts)

      def mutate(node, opts \\ []), do: Graphex.Repo.mutate(@name, node, opts)
      def mutate!(node, opts \\ []), do: Graphex.Repo.mutate!(@name, node, opts)

      def delete(node, opts \\ []), do: Graphex.Repo.delete(@name, node, opts)
      def delete!(node, opts \\ []), do: Graphex.Repo.delete!(@name, node, opts)

      def get(uid), do: Graphex.Repo.get(@name, meta(), uid)
      def get!(uid), do: Graphex.Repo.get!(@name, meta(), uid)

      def get_by(field, value), do: Graphex.Repo.get_by(@name, field, value)

      def all(query, params \\ %{}), do: Graphex.Repo.all(@name, query, params, meta())

      def meta(), do: Graphex.Repo.Meta.get(@meta_name)
      def register(modules), do: Graphex.Repo.Meta.register(@meta_name, modules)
      def snapshot(), do: Graphex.Repo.snapshot(@meta_name)
      def alter_schema(snapshot \\ snapshot()), do: Graphex.Repo.alter_schema(@name, snapshot)

      def stop(timeout \\ 5000), do: Supervisor.stop(@name, :normal, timeout)

      def drop_all(), do: Graphex.Repo.drop_all(@name)
    end
  end
  ...
  def child_spec(%{module: module, otp_app: otp_app, name: name, opts: opts}) do
    ...
    Graphex.child_spec([{:name, name} | opts])
  end
  ...
  def all(conn, query, params, %{lookup: lookup} = _meta \\ %{lookup: %{}}) do
    with {:ok, data} <- Graphex.query(conn, query, params), do: decode(data, lookup, false)
  end
  ...
  def mutate(conn, data, opts) do
    ...
    case encode(data_with_ids) do
      ...
      encoded_data ->
        with {:ok, %{uids: ids_map}} <- Graphex.set(conn, %{}, encoded_data, opts) do
  ...
  def delete(conn, data, _opts) do
    case encode(data) do
      ...
      encoded_data ->
        Graphex.delete(conn, encoded_data)
    end
  end
  ...
  def get(conn, %{lookup: lookup}, uid) do
    ...
    with {:ok, %{"uid_get" => nodes}} <- Graphex.query(conn, statement) do
      case nodes do
  ...
  def alter_schema(conn, snapshot) do
    with {:ok, sch} <- Graphex.query_schema(conn), do: do_alter_schema(conn, sch, snapshot)
  end
  ...
  defp do_alter_schema(conn, %{"schema" => schema, "types" => types}, snapshot) do
    ...
    case delta do
      %{"schema" => [], "types" => []} -> {:ok, 0}
      alter -> with {:ok, _} <- Graphex.alter(conn, %{schema: alter}), do: {:ok, delta_l}
    end
  end
