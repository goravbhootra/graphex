defmodule Graphex.Node do
  ...
  ## Usage
    ...
    defmodule User do
      use Graphex.Node, depends_on: Shared
  ...
  * `__schema__(:type, field)` - Returns the type of the given non-virtual field;
  ...
  defmacro __using__(opts) do
    ...
    quote do
      ...
      import Graphex.Node, only: [shared: 1, schema: 2]
    end
  end
  ...
  defp postprocess() do
    quote unquote: false do
      ...
      source = @name
      alter = Graphex.Node.__schema_alter___(__MODULE__, source)
      ...
      def __schema__(:depends_on), do: unquote(Graphex.Node.__depends_on_modules__(__MODULE__))

      for %Graphex.Field{name: name, type: type} <- @fields_data do
        def __schema__(:type, unquote(name)), do: unquote(type)
      end

      def __schema__(:type, _), do: nil

      for %Graphex.Field{name: name, db_name: db_name, type: type} <- @fields_data do
      ...
      changeset = Graphex.Node.__gen_changeset__(@fields_data)
  ...
  def __gen_changeset__(fields) do
    for %Graphex.Field{name: name, type: type} <- fields, into: %{}, do: {name, ecto_type(type)}
  end
  ...
  defmacro field(name, type, opts \\ []) do
    quote do
      Graphex.Node.__field__(__MODULE__, unquote(name), unquote(type), unquote(opts), @depends_on)
    end
  end
